...
import { OrderSource, type Prisma, type PrismaClient } from "@prisma/client";

/**
 * Deterministic demo-data generator, shared by the CLI seed (`prisma/seed.ts`)
 * and the admin reset endpoint (`/api/admin/reset`). Takes the Prisma client as
 * a parameter so it works both from a standalone tsx script and from inside the
 * Next runtime without importing app-only modules (no `@/` aliases here).
 *
 * It is the single source of the "Brewline" base: ~8,000 customers and ~35,000
 * ORGANIC orders over 18 months, shaped into VIPs / lapsed / one-time buyers so
 * the segment "high spenders gone quiet" returns a juicy count. Re-running wipes
 * ALL domain tables first, so the result is reproducible to the row.
 */
...
export type ReseedResult = {
  customers: number;
  orders: number;
  lapsedHighSpenders: number;
};

/**
 * Wipe every domain table and regenerate the deterministic Brewline base.
 * Idempotent and reproducible: same SEED ⇒ identical rows every run.
 */
export async function reseedDatabase(
  ...
  log(`Inserting ${customers.length} customers…`);
  ...
  const lapsedHighSpenders = customers.filter(
    (c) =>
      typeof c.totalSpend === "number" &&
      c.totalSpend >= 500000 &&
      c.lastOrderAt instanceof Date &&
      NOW.getTime() - c.lastOrderAt.getTime() > 90 * DAY,
  ).length;

  return { customers: customers.length, orders: orders.length, lapsedHighSpenders };
}
